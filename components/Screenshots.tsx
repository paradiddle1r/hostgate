  ...
  return (
    <div className="p-6">
      <div className="grid grid-cols-4 gap-3">
        {[
          { l: "Today Check-ins", v: "12", chip: "bg-emerald-50 text-emerald-700" },
          { l: "Check-outs", v: "8", chip: "bg-indigo-50 text-indigo-700" },
          { l: "Occupancy", v: "87%", chip: "bg-fuchsia-50 text-fuchsia-700" },
          { l: "Revenue", v: "฿128,400", chip: "bg-cyan-50 text-cyan-700" },
        ].map((k) => (
          <div key={k.l} className="rounded-xl border border-zinc-200 bg-white p-4">
            <div className="text-xs text-zinc-500">{k.l}</div>
            <div className="mt-1 text-xl font-bold text-zinc-900">{k.v}</div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        <div className="col-span-2 rounded-xl border border-zinc-200 bg-white p-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="text-sm font-semibold text-zinc-900">Revenue this month</div>
            <div className="rounded-md bg-zinc-100 px-2 py-0.5 text-[10px] text-zinc-600">May 2026</div>
          </div>
          <RevenueChart />
        </div>
        <div className="rounded-xl border border-zinc-200 bg-white p-4">
          <div className="mb-3 text-sm font-semibold text-zinc-900">Arrivals today</div>
          <div className="space-y-2">
            {[
              ...
              <div key={a.name} className="flex items-center justify-between rounded-md bg-zinc-50 px-2.5 py-1.5">
                <div>
                  <div className="text-xs text-zinc-900">{a.name}</div>
                  <div className="text-[10px] text-zinc-500">Room {a.room}</div>
                </div>
                <div className="text-[10px] text-zinc-600">{a.time}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="p-6">
      <div className="grid grid-cols-[60px_repeat(20,1fr)] gap-[2px] text-[10px]">
        ...
        {[101, 102, 103, 201, 202, 203, 301, 302, 303, 401].map((room, ri) => {
          const colors = ["bg-indigo-400", "bg-emerald-400", "bg-fuchsia-400", "bg-amber-400"];
          const rowBlocks = [
...
function RoomLine({ room, blocks }: { room: string; blocks: [number, number, string][] }) {
  return (
    <>
      <div className="py-1 text-zinc-600">Room {room}</div>
      {Array.from({ length: 20 }).map((_, i) => {
        const seg = blocks.find((b) => i >= b[0] && i <= b[1]);
        return (
          <div
            key={i}
            className={`h-6 rounded-[3px] ${seg ? seg[2] : "bg-zinc-100"}`}
          />
        );
      })}
    </>
  ...
  return (
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-zinc-900">Channel performance</h3>
          <p className="text-[11px] text-zinc-500">Last 30 days · all properties</p>
        </div>
        <div className="rounded-md bg-emerald-50 px-2.5 py-1 text-[10px] text-emerald-700">
          All channels synced
        </div>
      </div>
      <div className="space-y-2">
        {channels.map((c) => (
          <div key={c.name} className="flex items-center justify-between rounded-lg border border-zinc-200 bg-white p-3">
            <div className="flex items-center gap-3">
              <div className={`h-8 w-8 rounded-md ${c.color}`} />
              <div>
            ...
            <div className="flex items-center gap-6 text-right">
              <div>
                ...
                <div className="text-sm font-semibold text-zinc-900">{c.bookings}</div>
              </div>
              <div>
                <div className="text-[10px] text-zinc-500">Revenue</div>
                <div className="text-sm font-semibold text-zinc-900">{c.rev}</div>
              </div>
            </div>
          </div>
        ))}
...
function ReportsView() {
  return (
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <div className="text-sm font-semibold text-zinc-900">Performance · May 2026</div>
        <div className="flex gap-1.5">
          <span className="rounded-md bg-zinc-100 px-2 py-0.5 text-[10px] text-zinc-700">Export CSV</span>
          <span className="rounded-md bg-zinc-100 px-2 py-0.5 text-[10px] text-zinc-700">Export PDF</span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={m.l} className="rounded-xl border border-zinc-200 bg-white p-4">
            <div className="text-xs text-zinc-500">{m.l}</div>
            <div className="mt-1 flex items-baseline gap-2">
              <div className="text-2xl font-bold text-zinc-900">{m.v}</div>
              <div className="text-[10px] text-emerald-600">{m.trend}</div>
            </div>
            <MiniSpark />
          </div>
        ))}
