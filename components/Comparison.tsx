    ...
    <section className="relative py-24 lg:py-32">
      <div className="mx-auto max-w-5xl px-5 lg:px-8">
        ...
        <div className="mt-14 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02]">
          {/* Header row */}
          <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr] gap-px bg-white/5 text-xs sm:text-sm">
            <div className="bg-[#050816] px-4 py-4 font-semibold text-zinc-400 sm:px-6">
              {pick(t.comparison.cols.feature, locale)}
            </div>
            <div className="bg-gradient-to-b from-indigo-500/10 to-fuchsia-500/10 px-4 py-4 text-center font-bold text-white sm:px-6">
              {pick(t.comparison.cols.hostgate, locale)}
            </div>
            <div className="bg-[#050816] px-4 py-4 text-center text-zinc-300 sm:px-6">
          ...
          {t.comparison.rows.map((row, i) => (
            <div
              key={i}
              className="grid grid-cols-[1.4fr_1fr_1fr_1fr] gap-px bg-white/5 text-xs sm:text-sm"
            >
              <div className="bg-[#050816] px-4 py-3.5 text-zinc-200 sm:px-6">
...
function Cell({ variant, highlight }: { variant: string; highlight?: boolean }) {
  const bg = highlight ? "bg-gradient-to-b from-indigo-500/[0.06] to-fuchsia-500/[0.06]" : "bg-[#050816]";
  return (
    <div className={`flex items-center justify-center px-4 py-3.5 sm:px-6 ${bg}`}>
      {variant === "yes" ? (
        <svg viewBox="0 0 20 20" className="h-5 w-5 text-emerald-400" fill="currentColor">
          <path d="M16.7 5.3a1 1 0 010 1.4l-7.4 7.4a1 1 0 01-1.4 0L3.3 9.5A1 1 0 014.7 8.1l3 3 6.7-6.7a1 1 0 011.3-.1z" />
        </svg>
      ) : (
        <svg viewBox="0 0 20 20" className="h-5 w-5 text-zinc-600" fill="currentColor">
